...
class ImageNetLT_cb(ImageNetLT):
    def __init__(self, root, txt, transform=None):
        super().__init__(root, txt, transform)
        self.samples_idxs_per_class = {}
        self.max_num_samples = 0
        for idx, label in enumerate(self.labels):
            if label not in self.samples_idxs_per_class:
                self.samples_idxs_per_class[label] = []
            self.samples_idxs_per_class[label].append(self.img_path[idx])
            self.max_num_samples = max(self.max_num_samples, len(self.samples_idxs_per_class[label]))
        self.num_classes = len(self.samples_idxs_per_class.keys())

    def __len__(self):
        return self.max_num_samples * self.num_classes

    def __getitem__(self, index):
        label = index // self.max_num_samples
        sample_idx = (index % self.max_num_samples) % len(self.samples_idxs_per_class[label])
        path = self.samples_idxs_per_class[label][sample_idx]
